...
function findItemById(id) {
  ...
  const cart = items.find (item => item.id === id);
  return(cart);
...
function calculateTotalInventory() {
  ...
  let sum = items.reduce((prev,curr)=>
prev+curr.inventory,0)
  return sum
...
function getItemPriceByName(name) {
 ...
 let cart = items.find(item =>item.name === name)
 return cart.price
}
...
function filterItemsByCategoryId(categoryId) {
  ...
  let cart = items.filter((item)=>item.id === categoryId)
  return cart
...
function logCartItems() {
  ...
  //   let cart = idArr.map(function(item){
  //     return parseInt(item)
  //   })//change idArr string to number // cart = [1,3,5]
  //   //return console.log(cart);//test success
  //   // idArr.forEach((item=>console.log(item)))//test success
  //   let superCart = []// try to match items id with cart array 
